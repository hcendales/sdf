 ...
 */
public class rptLiqSimitExt extends AbstractPageBean {
    ...
    public String btReport_action() {
        try {
            ...
            PageReport =  Constants.darPathApp() + "ReportEngine?NameFilter=" + (String)ddFormato.getValue() +
                    ...
                    "&P_DIV_MUN=" + ddMunicipio.getSelected() +
                    "&rptfilename=" + "LiquidacionSimitExterno";
            
            if (this.checkbox1.getValue() != null )
                if (((String) this.checkbox1.getValue()).equals("1") )
                    PageReport =  Constants.darPathApp() + "ReportEngine?NameFilter=" + (String)ddFormato.getValue() +
                            "&P_FEC_INI=" + formatter.format(fi) +
                            "&P_FEC_FIN=" + formatter.format(ff) +
                            "&rptfilename=" + "LiquidacionSimitExternoTot";
            ExternalContext externalContext = getFacesContext().getExternalContext();
            
            externalContext.redirect(PageReport);
        } catch (Exception ex) {
            error("Failed to redirect to page " + ex.getMessage());
        }

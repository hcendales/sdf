 ...
 * init.java
 *
 * Created on 10 de octubre de 2007, 04:55 PM
 * Copyright DSOFTFIN
 */
package wasimit;
 ...
 */
public class init extends AbstractPageBean {
    ...
    public void setBtLogin(Button b) {
        this.btLogin = b;
    }
    ...
    private PasswordField tfpassw = new PasswordField();
    
    public PasswordField getTfpassw() {
        return tfpassw;
    }
    
    public void setTfpassw(PasswordField pf) {
        this.tfpassw = pf;
    }
    ...
    public void init() {
       ...
       // <editor-fold defaultstate="collapsed" desc="Managed Component Initialization">
        ...
        try {
            ...
            log("init Initialization Failure", e);
        ...
        if (Constants.AMBIENTE.equals("PROD")) {
            this.image2.setUrl("/resources/logo_simit.gif");
            this.image1.setUrl("/resources/logo_fcm.gif");
        } else
        {
            this.image2.setUrl("/resources/logo_simit1.gif");
            this.image1.setUrl("/resources/logo_fcm1.gif");
        }
    ...
    public String btLogin_action() {
        ...
        try {
            ...
            //validaciones
            if(tfuser.getValue().toString().length()==0)
                throw new Exception(" Ingrese Usuario");
            
            try{
                float vlr = Float.parseFloat((String)this.tfuser.getText());
            }catch(NumberFormatException ex) {
               throw new Exception("Usuario debe ser numerico");
            }
            
            if(tfpassw.getValue().toString().length()==0)
                throw new Exception("Clave obligatoria");
            
            con = getJdbcDataSource().getConnection();
            cs = con.prepareCall(Constants.validaUser);

                        
            // Cargamos los parametros de entrada IN
            cs.setString(++pos, (String)tfuser.getText());
            cs.setString(++pos, (String)this.tfpassw.getText());
            cs.setString(++pos, "FEDERACION COLOMBIANA DE MUNICIPIOS");
            ...
            getSessionBean1().setUser((String)tfuser.getText());
            getSessionBean1().setnameUser(cs.getString(5));

 ...
 */
public class Desencripta extends AbstractPageBean {
    ...
    public String btLiquidar_action() {
        ...
        try {
            ...
            if (uploadedFile == null)
                throw new Exception("Archivo a desencriptar no seleccionado");
            ...
            if (!extencionFileName.equals("pgp") &&  !extencionFileName.equals("gpg") )
            ...
            cs = con.prepareCall(Constants.carguearchivoDes);
            
            // Cargamos los parametros de entrada IN
            cs.setString("p_nombreArchivo", justFileName);
            
            lobsize = (int) uploadedFile.getSize();
            cs.setBinaryStream("p_archivo", uploadedFile.getInputStream(),lobsize);
            
            cs.setString("p_banco", ddBanco.getSelected().toString());
            
            cs.setString("p_usuario", getSessionBean1().getUser());
            
            // Registramos los parametro de salida OUT
            cs.registerOutParameter("p_codRespuesta", java.sql.Types.NUMERIC);
            cs.registerOutParameter("p_error", java.sql.Types.CHAR);
            cs.registerOutParameter("p_archivoDes", java.sql.Types.BLOB);
            cs.registerOutParameter("p_nomArchivoDes", java.sql.Types.CHAR);

            cs.execute();

            resultado = "" +  cs.getInt("p_codRespuesta") + "-" + cs.getString("p_error");

            info(resultado);

            if (cs.getInt("p_codRespuesta") != 0)
                return null;
             

            String nomArchivo = cs.getString("p_nomArchivoDes");
            Blob archivo = cs.getBlob("p_nomArchivoDes");
    ...
    try {
            ...
            cs = con.prepareCall(Constants.verLogDesencripta);
            ...
            logAvvillas.setText(cs.getString(2));

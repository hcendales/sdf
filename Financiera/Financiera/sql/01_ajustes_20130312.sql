...
-- Created by SEVEN on 12/03/2013, 16:19:10 --
----------------------------------------------

set define off
spool 01_ajustes_20130312.log

prompt
prompt Creating table FONDO
prompt ====================
prompt
drop table fondo;
...
comment on column NOVDEVOLUCION.vtercero
  is 'Valor metrotransito';
comment on column NOVDEVOLUCION.feccom
  is 'Fecha comparendo';
...
prompt
prompt Creating table PAGOFONDO
prompt ========================
prompt
drop table PAGOFONDO;

create table PAGOFONDO
(
  fechaaplicacion  DATE,
  tiporecaudo      CHAR(1),
  cuentaorigen     VARCHAR2(16),
  valor            NUMBER,
  fechanovedad     DATE,
  usuarionovedad   NUMBER,
  fechaliquidacion DATE,
  idreg            NUMBER not null
)
;
comment on column PAGOFONDO.tiporecaudo
  is 'L: Local E:Externo';
alter table PAGOFONDO
  add constraint PK_PAGOFONDO primary key (IDREG);

prompt
prompt Creating view VW_CUENTAORIGENFON
prompt ================================
prompt
create or replace force view vw_cuentaorigenfon as
select c.cuenta as cuenta,
c.cuenta || '-' || max(c.indtipo) || '-EXTERNO-' || max(b.nombre) as nomCuenta
...
union all
select c.cuenta,
c.cuenta || '-' || max(c.indtipo) || '-LOCAL-' || max(b.nombre) as nomCuenta
...
create or replace package body PK_NOVDEV is
  ...
  BEGIN
    ...
    IF min_fecha_act > w_fecha_act then
      V_ERROR := 'Fecha Activación Inválida: Minima debe ser ' || to_char(min_fecha_act, 'yyyy-mm-dd')  ;
      return;
    end if;
  ...
  begin
    ...
    if (V_INDTIPO = 'SIMIT') then
      ...
      IF (V_NUMREG > 0) THEN
        ...
        V_VFCMCONTRAVENCIONAL := V_VFCMCONTRAVENCIONAL + V_VIVA95 + V_VRET95;
        V_VCONCONTRAVENCIONAL := V_VCONCONTRAVENCIONAL + V_VIVA95 - V_VRET95;
      ...
      if ( V_IDMUNICIPIO = '08001000' and V_FECCOM < to_date('24/12/2008', 'dd/mm/yyyy')) then

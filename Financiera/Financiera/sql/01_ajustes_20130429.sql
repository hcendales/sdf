...
-- Created by SEVEN on 29/04/2013, 15:31:10 --
----------------------------------------------

set define off
spool 01_ajustes_20130429.log
...
create or replace package body PK_NOVDEV is
  ...
  begin
    ...
    if (V_INDTIPO = 'SIMIT') then
      ...
      if (V_VALORBASE < v_salario and v_indretro = 1 and V_FECCOM >= to_date('2002-11-08','yyyy-mm') ) then
        if ((V_VDEVOLUCION - V_ADICIONAL) < v_salario) then
          V_VALORBASE := V_VDEVOLUCION - V_ADICIONAL;
        else
          V_VALORBASE := V_SALARIO;
